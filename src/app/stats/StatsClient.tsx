...
export default function StatsClient({ mode }: { mode: StatsMode }) {
  const { address, isConnected } = useAccount();
  const [data, setData] = useState<StatsSummary | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const wallet = mode === 'my' && isConnected && address ? address : undefined;
  const showMySignInPrompt = mode === 'my' && !wallet;
  ...
  const marketCategory = useMemo(() => new Map(MARKETS.map((m) => [m.id, m.category])), []);
    ...
    <TerminalShell
      ...
      leftPane={
        <div className={styles.pane}>
          ...
          {showMySignInPrompt ? (
            <div className={styles.muted}>CONNECT WALLET TO VIEW PERSONAL STATS.</div>
          ) : null}
      ...
      rightPane={
        <div className={styles.pane}>
          <TerminalPaneTitle>HISTORY</TerminalPaneTitle>
          <button
            type="button"
