import { deadDropToken, deadDropTokensForContacts } from '@shadowbroker/mesh/meshDeadDrop';
import { mailboxClaimToken, mailboxDecoySharedToken } from '@shadowbroker/mesh/meshMailbox';
import {
  deriveSenderSealKey,
  ensureDhKeysFresh,
  deriveSharedKey,
...
import { PROTOCOL_VERSION, type JsonValue } from '@shadowbroker/mesh/meshProtocol';
import {
  ensureCanonicalRequestV2SenderSeal,
  requiresCanonicalRequestV2SenderSeal,
} from '@shadowbroker/mesh/requestSenderSealPolicy';
...
export async function ensureRegisteredDmKey(
  ...
  const res = await fetch(`${apiBase}/mesh/dm/register`, {
    method: 'POST',
...
export async function fetchDmPublicKey(
  ...
  const res = await fetch(`${apiBase}/mesh/dm/pubkey?agent_id=${encodeURIComponent(agentId)}`);
  const data = await res.json();
...
async function signedMailboxRequest(
  ...
  const res = await fetch(`${apiBase}/mesh/dm/${eventType === 'dm_poll' ? 'poll' : 'count'}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      agent_id: signed.context.nodeId,
...
export async function sendDmMessage(request: DmSendRequest): Promise<DmSendResponse> {
  ...
  const res = await fetch(`${request.apiBase}/mesh/dm/send`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
