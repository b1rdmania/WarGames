import { sbApi } from '@shadowbroker/lib/api';
import {
...
} from '@shadowbroker/lib/desktopControlContract';
import { commandToHttpRequest } from '@shadowbroker/lib/desktopControlRouting';
import type { ShadowbrokerDesktopRuntime } from '@shadowbroker/lib/desktopBridge';
...
export function createHttpBackedDesktopRuntime(): ShadowbrokerDesktopRuntime {
  ...
  return {
    async invokeLocalControl<T = unknown>(
      ...
      const res = await fetch(sbApi(request.path), {
