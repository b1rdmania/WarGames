...
import { onTileLoadingChange, resetTileLoading } from '@shadowbroker/lib/sentinelHub';
...
import type {
  ...
  Scanner,
  TrackedFlight,
} from '@shadowbroker/types/dashboard';
...
function ScannerTracker({
  ...
  const fetchAndPlay = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await fetch(`${API_BASE}/radio/openmhz/calls/${scanner.shortName}`);
      if (!res.ok) {
        setIsLoading(false);
        return;
      }
