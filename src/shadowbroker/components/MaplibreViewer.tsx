...
import { computeNightPolygon } from '@shadowbroker/utils/solarTerminator';
import { darkStyle, lightStyle } from '@shadowbroker/components/map/styles/mapStyles';
import maplibregl from 'maplibre-gl';
import { AlertTriangle, Radio, Activity, Play, Pause, Satellite } from 'lucide-react';
import HlsVideo, { type HlsVideoHandle } from '@shadowbroker/components/HlsVideo';
import WikiImage from '@shadowbroker/components/WikiImage';
import ExternalImage from '@shadowbroker/components/ExternalImage';
import { useTheme } from '@shadowbroker/lib/ThemeContext';
...
import {
  ...
  VOLCANO_ICON_SPECS,
  WEATHER_ICON_SPECS,
} from '@shadowbroker/components/map/icons/AircraftIcons';
import { makeSatSvg, makeISSSvg, makeTrainSvg } from '@shadowbroker/components/map/icons/SatelliteIcons';
import { EMPTY_FC } from '@shadowbroker/components/map/mapConstants';
import { useImperativeSource } from '@shadowbroker/components/map/hooks/useImperativeSource';
import { useDynamicMapLayersWorker } from '@shadowbroker/components/map/hooks/useDynamicMapLayersWorker';
import { useStaticMapLayersWorker } from '@shadowbroker/components/map/hooks/useStaticMapLayersWorker';
import {
  ClusterCountLabels,
  TrackedFlightLabels,
  CarrierLabels,
  ...
  EarthquakeLabels,
  ThreatMarkers,
} from '@shadowbroker/components/map/MapMarkers';
import type { KiwiSDR, MaplibreViewerProps, Scanner, SigintSignal } from '@shadowbroker/types/dashboard';
import { useDataSnapshot } from '@shadowbroker/hooks/useDataStore';
import { useInterpolation } from '@shadowbroker/components/map/hooks/useInterpolation';
import { useClusterLabels } from '@shadowbroker/components/map/hooks/useClusterLabels';
import { spreadAlertItems } from '@shadowbroker/utils/alertSpread';
import { SigintSendForm, MeshtasticChannelFeed } from '@shadowbroker/components/map/panels/SigintPanels';
import { useViewportBounds } from '@shadowbroker/components/map/hooks/useViewportBounds';
import { MeasurementLayers } from '@shadowbroker/components/map/layers/MeasurementLayers';
import {
...
} from '@shadowbroker/components/map/geoJSONBuilders';
...
const MaplibreViewer = ({
  ...
  const data = useDataSnapshot() as import('@shadowbroker/types/dashboard').DashboardData;
  ...
  useEffect(() => {
    ...
    if (callsign && callsign !== prevCallsign.current) {
      ...
      fetch(`${API_BASE}/route/${callsign}?lat=${entityLat}&lng=${entityLng}`)
        .then((res) => res.json())
        .then((routeData) => {
  ...
  useEffect(() => {
    ...
    let alive = true;
    fetch(`${API_BASE}/oracle/region-intel?lat=${lat}&lng=${lng}`)
      .then(r => r.json())
                  ...
                  <div className="flex items-center gap-2 mt-3 pt-2 border-t border-[var(--border-primary)]">
                    ...
                    <button
                      onClick={async (e) => {
                        ...
                        if (sn) {
                          try {
                            const res = await fetch(`${API_BASE}/radio/openmhz/calls/${sn}`);
                            if (res.ok) {
