...
import { controlPlaneFetch } from '@shadowbroker/lib/controlPlane';
import {
  requestMeshTerminalOpen,
  subscribeSecureMeshTerminalLauncherOpen,
} from '@shadowbroker/lib/meshTerminalLauncher';
import { purgeBrowserContactGraph, purgeBrowserSigningMaterial, setSecureModeCached, getNodeIdentity, generateNodeKeys } from '@shadowbroker/mesh/meshIdentity';
      ...
      {!['available', 'confirming', 'updating', 'restarting', 'update_error', 'docker_update'].includes(
        ...
        >
          {updateStatus === 'checking' && (
            <ExternalLink size={11} className="animate-spin text-cyan-400" />
          )}
          {updateStatus === 'idle' && <ExternalLink size={11} className="text-cyan-400" />}
