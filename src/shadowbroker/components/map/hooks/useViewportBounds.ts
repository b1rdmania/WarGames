...
import {
  ...
  normalizeViewBounds,
  type ViewBounds,
} from '@shadowbroker/lib/viewportPrivacy';
...
export function useViewportBounds(
  ...
  const postViewport = useCallback((bounds: ViewBounds) => {
    lastPostedBoundsRef.current = bounds;
  ...
  const updateBounds = useCallback(() => {
    ...
    debounceTimerRef.current = setTimeout(() => {
      ...
      if (!lastPostedBoundsRef.current) {
        postViewport(preloadBounds);
        return;
      }
      const now = Date.now();
      if (
  ...
  }, [backendViewportSyncEnabled, mapRef, postViewport, viewBoundsRef]);
