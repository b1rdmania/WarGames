...
import { useGateSSE } from '@shadowbroker/hooks/useGateSSE';
import { requestSecureMeshTerminalLauncherOpen } from '@shadowbroker/lib/meshTerminalLauncher';
import {
  loadIdentityBoundSensitiveValue,
  persistIdentityBoundSensitiveValue,
} from '@shadowbroker/lib/identityBoundSensitiveStorage';
import {
  getDesktopNativeControlAuditReport,
} from '@shadowbroker/lib/desktopBridge';
import { describeNativeControlError } from '@shadowbroker/lib/desktopControlContract';
import type { DesktopControlAuditReport } from '@shadowbroker/lib/desktopControlContract';
import { fetchPrivacyProfileSnapshot } from '@shadowbroker/mesh/controlPlaneStatusClient';
import {
...
import {
  ...
  ratchetDecryptDM,
  ratchetReset,
} from '@shadowbroker/mesh/meshDmWorkerClient';
import {
  bootstrapDecryptAccessRequest,
  bootstrapEncryptAccessRequest,
  canUseWormholeBootstrap,
} from '@shadowbroker/mesh/wormholeDmBootstrapClient';
import {
...
import { fetchWormholeSettings, joinWormhole, leaveWormhole } from '@shadowbroker/mesh/wormholeClient';
import {
  buildMailboxClaims,
  countDmMailboxes,
...
import {
  ...
  preferredDmPeerId,
} from '@shadowbroker/mesh/meshDmConsent';
import { deriveSasPhrase } from '@shadowbroker/mesh/meshSas';
import { PROTOCOL_VERSION } from '@shadowbroker/mesh/meshProtocol';
...
} from '@shadowbroker/mesh/meshPrivacyHints';
import {
...
} from '@shadowbroker/mesh/requestSenderRecovery';
import type { SenderRecoveryState } from '@shadowbroker/mesh/requestSenderRecovery';
...
const MeshChat = React.memo(function MeshChat({
  ...
  const handleLocateUser = async (callsign: string) => {
    ...
    if (!onFlyTo) return;
    try {
      const res = await fetch(`${API_BASE}/mesh/signals?source=meshtastic&limit=500`);
      if (res.ok) {
  ...
  useEffect(() => {
    ...
    const fetchGates = async () => {
      try {
        const res = await fetch(`${API_BASE}/mesh/gate/list`);
        if (res.ok) {
          const data = await res.json();
          setGates(data.gates || []);
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        const res = await fetch(`${API_BASE}/mesh/infonet/messages?${params}`, {
          headers,
        });
        if (res.ok && !cancelled) {
          const data = await res.json();
          ...
          if (uncachedNodeIds.length > 0) {
            try {
              ...
              const r = await fetch(`${API_BASE}/mesh/reputation/batch?${repParams.toString()}`);
              if (r.ok) {
  ...
  useEffect(() => {
    ...
    const fetchChannels = async () => {
      try {
        const res = await fetch(`${API_BASE}/mesh/channels`);
        if (res.ok && !cancelled) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        const res = await fetch(`${API_BASE}/mesh/messages?${params}`);
        if (res.ok && !cancelled) {
          const data = await res.json();
    ...
    const run = async () => {
      ...
      try {
        const witnessRes = await fetch(
          `${API_BASE}/mesh/dm/witness?target_id=${encodeURIComponent(
            selectedContact,
        ...
        const vouchRes = await fetch(
          `${API_BASE}/mesh/trust/vouches?node_id=${encodeURIComponent(selectedContact)}`,
        );
  ...
  const handleSend = async () => {
    ...
    try {
        if (activeTab === 'infonet' && selectedGate) {
          const gateReplyPrefix =
            gateReplyContext && gateReplyContext.gateId === String(selectedGate).trim().toLowerCase()
              ...
              : '';
          const gateRes = await fetch(`${API_BASE}/wormhole/gate/message/post`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
        ...
        setGateReplyContext(null);
          ...
          const sendRes = await fetch(`${API_BASE}/mesh/send`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              destination: meshDestination,
        ...
        const mRes = await fetch(`${API_BASE}/mesh/messages?${params}`);
        if (mRes.ok) {
  ...
  const handleVote = async (targetId: string, vote: 1 | -1, gateIdOverride?: string) => {
    ...
    try {
      ...
      await fetch(`${API_BASE}/mesh/vote`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      const res = await fetch(
        `${API_BASE}/mesh/reputation?node_id=${encodeURIComponent(targetId)}`,
      );
      if (res.ok) {
        const data = await res.json();
  ...
  const handleCreateGate = async () => {
    ...
    try {
      ...
      const createRes = await fetch(`${API_BASE}/mesh/gate/create`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const refreshDmContactState = async (
    ...
    try {
      const vouchRes = await fetch(
        `${API_BASE}/mesh/trust/vouches?node_id=${encodeURIComponent(targetId)}`,
      );
      if (vouchRes.ok) {
        const vouchData = await vouchRes.json();
        updateContact(targetId, {
  ...
  const handleAcceptRequest = async (senderId: string) => {
    ...
    try {
      // Fetch their pubkey
      const res = await fetch(
        `${API_BASE}/mesh/dm/pubkey?agent_id=${encodeURIComponent(senderId)}`,
      );
        if (res.ok) {
          const data = await res.json();
          if (data.dh_pub_key) {
            ...
            try {
              ...
              if (wValid.ok) {
                ...
                await fetch(`${API_BASE}/mesh/dm/witness`, {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({
              ...
              const vouchRes = await fetch(
                `${API_BASE}/mesh/trust/vouches?node_id=${encodeURIComponent(senderId)}`,
              );
              if (vouchRes.ok) {
                const vouchData = await vouchRes.json();
                updateContact(senderId, {
  ...
  const handleBlockDM = async (agentId: string) => {
    ...
    try {
      ...
      await fetch(`${API_BASE}/mesh/dm/block`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleVouch = async (targetId: string) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/mesh/trust/vouch`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleAddContact = async () => {
    ...
    if (!cid || !hasId) return;
    try {
        const res = await fetch(`${API_BASE}/mesh/dm/pubkey?agent_id=${encodeURIComponent(cid)}`);
        if (res.ok) {
          const data = await res.json();
          if (data.dh_pub_key) {
            ...
            try {
              ...
              const vouchRes = await fetch(
                `${API_BASE}/mesh/trust/vouches?node_id=${encodeURIComponent(cid)}`,
              );
              if (vouchRes.ok) {
                const vouchData = await vouchRes.json();
                updateContact(cid, {
