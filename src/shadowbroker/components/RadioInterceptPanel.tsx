...
import type { DashboardData, SelectedEntity, RadioFeed, SigintSignal } from '@shadowbroker/types/dashboard';
...
export default function RadioInterceptPanel({
  ...
  useEffect(() => {
    const fetchFeeds = async () => {
      try {
        const res = await fetch(`${API_BASE}/radio/top`);
        if (res.ok) {
      ...
      const fetchNearest = async () => {
        try {
          ...
          const res = await fetch(
            `${API_BASE}/radio/nearest?lat=${eavesdropLocation.lat}&lng=${eavesdropLocation.lng}`,
          );
          if (res.ok) {
            ...
            if (system && system.shortName) {
              ...
              const callRes = await fetch(
                `${API_BASE}/radio/openmhz/calls/${system.shortName}`,
              );
              if (callRes.ok) {
  ...
  const scanNextFeed = async () => {
    ...
    if (scanLoc) {
      try {
        const res = await fetch(
          `${API_BASE}/radio/nearest-list?lat=${scanLoc.lat}&lng=${scanLoc.lng}&limit=3`,
        );
        if (res.ok) {
