...
import {
  ...
  type NodeIdentity,
} from '@shadowbroker/mesh/meshIdentity';
import { ratchetEncryptDM, ratchetDecryptDM } from '@shadowbroker/mesh/meshDmWorkerClient';
import {
  buildMailboxClaims,
  countDmMailboxes,
  ...
  sendDmMessage,
  sharedMailboxToken,
} from '@shadowbroker/mesh/meshDmClient';
import { PROTOCOL_VERSION, buildSignaturePayload } from '@shadowbroker/mesh/meshProtocol';
import { validateEventPayload } from '@shadowbroker/mesh/meshSchema';
import { verifyMerkleProof } from '@shadowbroker/mesh/meshMerkle';
import { API_BASE } from '@shadowbroker/lib/api';
import { controlPlaneJson } from '@shadowbroker/lib/controlPlane';
import { getPrivacyStrictPreference } from '@shadowbroker/lib/privacyBrowserStorage';
import { getDesktopNativeControlAuditReport } from '@shadowbroker/lib/desktopBridge';
import { describeNativeControlError } from '@shadowbroker/lib/desktopControlContract';
import {
  getSensitiveBrowserItem,
  setSensitiveBrowserItem,
} from '@shadowbroker/lib/privacyBrowserStorage';
import {
  fetchInfonetNodeStatusSnapshot,
  type InfonetNodeStatusSnapshot,
} from '@shadowbroker/mesh/controlPlaneStatusClient';
import { fetchWormholeStatus } from '@shadowbroker/mesh/wormholeIdentityClient';
import {
...
} from '@shadowbroker/mesh/wormholeIdentityClient';
import { fetchWormholeSettings, joinWormhole } from '@shadowbroker/mesh/wormholeClient';
import {
  getMeshTerminalWriteLockReason,
  isMeshTerminalWriteCommand,
} from '@shadowbroker/lib/meshTerminalPolicy';
import {
  gateEnvelopeDisplayText,
  gateEnvelopeState,
  isEncryptedGateEnvelope,
} from '@shadowbroker/mesh/gateEnvelope';
...
export default function MeshTerminal({ isOpen, launchToken = 0, onClose, onDmCount, onSettingsClick }: Props) {
  ...
  useEffect(() => {
    ...
    const loadGateCatalog = async () => {
      ...
      try {
        const res = await fetch(`${API}/mesh/gate/list`);
  ...
  useEffect(() => {
    ...
    const loadMarkets = async () => {
      ...
      try {
        const res = await fetch(`${API}/mesh/oracle/markets`);
        const data = await res.json();
        if (cancelled) return;
          ...
          try {
            ...
            if (!senderDH) {
              const keyRes = await fetch(
                `${API}/mesh/dm/pubkey?agent_id=${encodeURIComponent(message.sender_id)}`,
              );
  ...
  const cmdSignals = useCallback(
    async (n: number = 10) => {
      try {
        ...
        const res = await fetch(`${API}/mesh/signals?limit=${limit}`);
        const data = await res.json();
  ...
  const cmdNews = useCallback(
    async (query: string, limit: number = 8) => {
      try {
        const res = await fetch(`${API}/live-data/slow`);
        const data = await res.json();
  ...
  const cmdJets = useCallback(
    async (query: string) => {
      ...
      try {
        const res = await fetch(`${API}/live-data/fast`);
        const data = await res.json();
        const sources = [
    ...
    async (query: string) => {
      ...
      try {
        const [slowRes, fastRes, marketRes, signalRes] = await Promise.all([
          fetch(`${API}/live-data/slow`),
          fetch(`${API}/live-data/fast`),
          fetch(`${API}/mesh/oracle/search?q=${encodeURIComponent(query)}&limit=5`),
          fetch(`${API}/mesh/signals?limit=80`),
        ]);
  ...
  const cmdShodan = useCallback(
    async (args: string[]) => {
      try {
        if (!args.length) {
          const res = await fetch(`${API}/tools/shodan/status`);
          const data = await res.json();
          addSystem('');
          addSystem('  SHODAN');
        ...
        if (args[0]?.toLowerCase() === 'host') {
          ...
          const res = await fetch(`${API}/tools/shodan/host`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        const res = await fetch(`${API}/tools/shodan/search`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const cmdMerkle = useCallback(async () => {
    try {
      const res = await fetch(`${API}/mesh/infonet/merkle`);
      const data = await res.json();
      addSystem('');
  ...
  const cmdRevoke = useCallback(
    async (args: string[]) => {
      ...
      try {
        ...
        const res = await fetch(`${API}/mesh/identity/revoke`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
  ...
  const cmdRotate = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/mesh/identity/rotate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          old_node_id: nodeIdentity.nodeId,
      ...
      addSystem(`  Encrypting for ${recipientId}...`);
        try {
          ...
          if (!theirDHPub) {
            // Fetch from server
            const keyRes = await fetch(
              `${API}/mesh/dm/pubkey?agent_id=${encodeURIComponent(recipientId)}`,
            );
            const keyData = await keyRes.json();
            if (!keyData.ok || !keyData.dh_pub_key) {
  ...
  const cmdNearest = useCallback(
    async (lat: string, lng: string) => {
      ...
      try {
        const res = await fetch(`${API}/sigint/nearest-sdr?lat=${lat}&lng=${lng}`);
        const data = await res.json();
  ...
  const postGateMessage = useCallback(
    async (gateId: string, plaintext: string) => {
      const sendAttempt = async () => {
        const res = await fetch(`${API}/wormhole/gate/message/post`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            gate_id: gateId,
    ...
    async (gateId: string) => {
      ...
      try {
        ...
        const [gateData, keyData, messageData] = await Promise.all([
          fetch(`${API}/mesh/gate/${encodeURIComponent(gateId)}`).then((res) => res.json()),
          fetchWormholeGateKeyStatus(gateId).catch(() => null),
          fetch(`${API}/mesh/infonet/messages?gate=${encodeURIComponent(gateId)}&limit=6`, {
            headers: gateHeaders,
          }).then((res) => res.json()),
        ]);
      ...
      try {
        ...
        switch (cmd.toLowerCase()) {
            ...
            if (!sub) {
              ...
              try {
                const res = await fetch(
                  `${API}/mesh/signals?source=meshtastic&region=${meshRegion}&limit=${n}`,
                );
                const data = await res.json();
          ...
          case 'rep': {
            ...
            try {
              const res = await fetch(
                `${API}/mesh/reputation?node_id=${encodeURIComponent(targetId)}`,
              );
              const data = await res.json();
              if (data.ok === false) {
          ...
          case 'myrep': {
            ...
            try {
              const res = await fetch(
                `${API}/mesh/reputation?node_id=${encodeURIComponent(nodeIdentity.nodeId)}`,
              );
              const data = await res.json();
              addSystem('');
          ...
          case 'markets': {
            try {
              ...
              const res = await fetch(
                query
                  ? `${API}/mesh/oracle/search?q=${encodeURIComponent(query)}&limit=100`
                  : `${API}/mesh/oracle/markets`,
              );
              const data = await res.json();
              const markets = query
          ...
          case 'predict': {
            ...
            if (!['yes', 'no'].includes(pSide)) {
              ...
              try {
                ...
                const res = await fetch(`${API}/mesh/oracle/predict`, {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({
                    node_id: nodeIdentity.nodeId,
          ...
          case 'oracle': {
            ...
            try {
              const res = await fetch(
                `${API}/mesh/oracle/profile?node_id=${encodeURIComponent(oTarget)}`,
              );
              const data = await res.json();
              if (data.ok === false) {
                addError(data.detail);
          ...
          case 'stake': {
            ...
            if (
              ...
              try {
                ...
                const res = await fetch(`${API}/mesh/oracle/stake`, {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({
          ...
          case 'stakes': {
            ...
            try {
              const res = await fetch(
                `${API}/mesh/oracle/stakes/${encodeURIComponent(stMsgId)}`,
              );
              const data = await res.json();
              addSystem('');
          ...
          case 'messages': {
            ...
            try {
              const url = msgGate
                ? `${API}/mesh/infonet/messages?gate=${encodeURIComponent(msgGate)}&limit=20`
                : `${API}/mesh/infonet/messages?limit=20`;
          ...
          case 'event': {
            ...
            try {
              const res = await fetch(`${API}/mesh/infonet/event/${encodeURIComponent(evtId)}`);
              const data = await res.json();
              if (data.ok === false) {
                addError(data.detail);
                break;
          ...
          case 'ledger': {
            ...
            try {
              const res = await fetch(
                `${API}/mesh/infonet/node/${encodeURIComponent(ledgerTarget)}?limit=20`,
              );
              const data = await res.json();
              const events = data.events || [];
              addSystem('');
          ...
          case 'dm': {
            ...
            if (sub === 'add') {
              ...
              try {
                const res = await fetch(
                  `${API}/mesh/dm/pubkey?agent_id=${encodeURIComponent(targetId)}`,
                );
                const data = await res.json();
            ...
            try {
              ...
              for (const m of msgs) {
                ...
                try {
                  ...
                  if (!senderDH) {
                    const keyRes = await fetch(
                      `${API}/mesh/dm/pubkey?agent_id=${encodeURIComponent(m.sender_id)}`,
                    );
                    const keyData = await keyRes.json();
                    if (keyData.ok && keyData.dh_pub_key) {
                      senderDH = keyData.dh_pub_key as string;
