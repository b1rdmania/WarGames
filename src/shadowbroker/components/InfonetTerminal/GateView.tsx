...
import { nextSequence } from '@shadowbroker/mesh/meshIdentity';
import {
...
import { gateEnvelopeDisplayText, gateEnvelopeState, isEncryptedGateEnvelope } from '@shadowbroker/mesh/gateEnvelope';
...
export default function GateView({
  ...
  const refreshGate = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/mesh/infonet/messages?${params}`, { headers });
      ...
      if (uniqueEventIds.length > 0) {
        try {
          ...
          const repRes = await fetch(`${API_BASE}/mesh/reputation/batch?${params}`);
          if (repRes.ok) {
  ...
  const handleVote = useCallback(async (eventId: string, vote: 1 | -1) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/mesh/vote`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
