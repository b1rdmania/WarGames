  ...
  useEffect(() => {
    ...
    const loadProfileStats = async () => {
      ...
      const [repResult, oracleResult] = await Promise.allSettled([
        fetch(`${API_BASE}/mesh/reputation?node_id=${encodeURIComponent(nodeId)}`, { cache: 'no-store' }),
        fetch(`${API_BASE}/mesh/oracle/profile?node_id=${encodeURIComponent(nodeId)}`, { cache: 'no-store' }),
      ]);
