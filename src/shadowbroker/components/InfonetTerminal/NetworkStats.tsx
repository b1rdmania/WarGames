...
import { fetchInfonetNodeStatusSnapshot } from '@shadowbroker/mesh/controlPlaneStatusClient';
    ...
    const poll = async () => {
      try {
        const [meshRes, channelsRes, infonet] = await Promise.all([
          fetch(`${API_BASE}/mesh/status`).then(r => r.ok ? r.json() : null).catch(() => null),
          fetch(`${API_BASE}/mesh/channels`).then(r => r.ok ? r.json() : null).catch(() => null),
