...
import { getNodeIdentity, nextSequence } from '@shadowbroker/mesh/meshIdentity';
import { validateEventPayload } from '@shadowbroker/mesh/meshSchema';
import { getActiveSigningContext, signMeshEvent } from '@shadowbroker/mesh/wormholeIdentityClient';
import { useDataKeys } from '@shadowbroker/hooks/useDataStore';
...
const PredictionsPanel = React.memo(function PredictionsPanel() {
  ...
  const fetchMarkets = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/mesh/oracle/markets`);
      if (res.ok) {
    ...
    async (cat: MarketCategory) => {
      ...
      try {
        const res = await fetch(
          `${API_BASE}/mesh/oracle/markets/more?category=${cat}&offset=${current}&limit=10`,
        );
        if (res.ok) {
          const d = await res.json();
          if (d.markets?.length) {
  ...
  const searchMarkets = useCallback(async (query: string) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/mesh/oracle/search?q=${encodeURIComponent(query)}&limit=20`,
      );
      if (res.ok) {
        const d = await res.json();
  ...
  const fetchPredictions = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/mesh/oracle/predictions?node_id=${encodeURIComponent(nodeId)}`,
      );
      if (res.ok) {
        const d = await res.json();
  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/mesh/oracle/profile?node_id=${encodeURIComponent(nodeId)}`,
      );
      if (res.ok) {
        const d = await res.json();
        setProfile(d);
  ...
  const placePrediction = async (marketTitle: string, side: string, stakeAmount: number = 0) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/mesh/oracle/predict`, {
        method: 'POST',
