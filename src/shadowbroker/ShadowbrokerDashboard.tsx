...
import WorldviewLeftPanel from '@shadowbroker/components/WorldviewLeftPanel';

import NewsFeed from '@shadowbroker/components/NewsFeed';
import MarketsPanel from '@shadowbroker/components/MarketsPanel';
import FilterPanel from '@shadowbroker/components/FilterPanel';
import FindLocateBar from '@shadowbroker/components/FindLocateBar';
import TopRightControls from '@shadowbroker/components/TopRightControls';
import PredictionsPanel from '@shadowbroker/components/PredictionsPanel';
import SettingsPanel from '@shadowbroker/components/SettingsPanel';
import MapLegend from '@shadowbroker/components/MapLegend';
import ScaleBar from '@shadowbroker/components/ScaleBar';
import MeshTerminal from '@shadowbroker/components/MeshTerminal';
import MeshChat from '@shadowbroker/components/MeshChat';
import InfonetTerminal from '@shadowbroker/components/InfonetTerminal';
import { leaveWormhole, fetchWormholeState } from '@shadowbroker/mesh/wormholeClient';
import ShodanPanel from '@shadowbroker/components/ShodanPanel';
import GlobalTicker from '@shadowbroker/components/GlobalTicker';
import ErrorBoundary from '@shadowbroker/components/ErrorBoundary';
import OnboardingModal, { useOnboarding } from '@shadowbroker/components/OnboardingModal';
import ChangelogModal, { useChangelog } from '@shadowbroker/components/ChangelogModal';
import type { ActiveLayers, KiwiSDR, Scanner, SelectedEntity } from '@shadowbroker/types/dashboard';
import type { ShodanSearchMatch } from '@shadowbroker/types/shodan';
import { NOMINATIM_DEBOUNCE_MS } from '@shadowbroker/lib/constants';
import { API_BASE } from '@shadowbroker/lib/api';
import { useDataPolling, LAYER_TOGGLE_EVENT } from '@shadowbroker/hooks/useDataPolling';
import { useBackendStatus, useDataKey } from '@shadowbroker/hooks/useDataStore';
import { useReverseGeocode } from '@shadowbroker/hooks/useReverseGeocode';
import { useRegionDossier } from '@shadowbroker/hooks/useRegionDossier';
import {
  requestSecureMeshTerminalLauncherOpen,
  subscribeMeshTerminalOpen,
} from '@shadowbroker/lib/meshTerminalLauncher';
import {
  hasSentinelInfoBeenSeen,
  markSentinelInfoSeen,
  hasSentinelCredentials,
  getSentinelUsage,
} from '@shadowbroker/lib/sentinelHub';

// Use dynamic loads for Maplibre to avoid SSR window is not defined errors
const MaplibreViewer = dynamic(() => import('@shadowbroker/components/MaplibreViewer'), { ssr: false });
    ...
    timerRef.current = setTimeout(async () => {
      ...
      try {
        ...
        const res = await fetch(
          `${API_BASE}/geocode/search?q=${encodeURIComponent(q)}&limit=5`,
          { signal },
        );
        if (res.ok) {
...
export default function Dashboard({ framed = false }: { framed?: boolean } = {}) {
  ...
  const [activeLayers, setActiveLayers] = useState<ActiveLayers>({
    ...
    tracked: true,
    gps_jamming: true,
    // Maritime — keep the cheap, high-signal layers on by default
    ships_military: true,
    ships_cargo: true,
  ...
  const [shodanStyle, setShodanStyle] = useState<import('@shadowbroker/types/shodan').ShodanStyleConfig>({ shape: 'circle', color: '#16a34a', size: 'md' });
  useDataPolling();
  ...
  return (
    <>
      <main
        className={`${framed ? 'absolute inset-0' : 'fixed inset-0'} w-full h-full bg-[var(--bg-primary)] overflow-hidden font-sans`}
      >
          ...
          <>
            ...
            >
              ...
              <div className="flex flex-col">
                <h1
                  className="text-2xl font-bold tracking-[0.34em] text-[var(--text-primary)] flex items-center gap-3 text-glow"
